...
* **New Resource:** `barndoor_mcp_server_publication` — publishes an MCP server, the one-way, admin-initiated step that makes it discoverable to end users. It is a separate resource (not a flag on `barndoor_mcp_server`) because publishing requires an ACTIVE policy, and policies reference the server's id — so the publication is ordered after them with `depends_on`. Re-applying is an idempotent no-op; removing the declaration does **not** unpublish (unpublishing does not exist) and never touches the server; import by server id (BCP-3734).
...
* resource/`barndoor_mcp_server`, data-source/`barndoor_mcp_server`: new read-only `published_at` attribute (RFC 3339; null while unpublished), so operators can see and assert whether a server is published (BCP-3734).
