...
* **New Resource:** `barndoor_mcp_server_publication` — publishes an MCP server, the one-way, admin-initiated step that makes it discoverable to end users. It is a separate resource (not a flag on `barndoor_mcp_server`) because publishing requires an ACTIVE policy, and policies reference the server's id — so the publication is ordered after them with `depends_on`. Re-applying is an idempotent no-op; removing the declaration does **not** unpublish (unpublishing does not exist) and never touches the server; import by server id. Requires platform support for the server publish endpoint (bdai-platform BCP-3039 / #6996) (BCP-3734).
...
* resource/`barndoor_mcp_server`, data-source/`barndoor_mcp_server`: new read-only `published_at` attribute (RFC 3339; null while unpublished), so operators can see and assert whether a server is published. Requires platform support for `published_at` on the server response (bdai-platform BCP-3039 / #6996) (BCP-3734).
* Nightly acceptance coverage now asserts publish-driven discoverability: a published server appears in the published listing (`availability_status=true`) and an unpublished one does not. The end-user audience-scoped listing cannot be asserted by the provider's machine credential (it has no user record) and is covered by the platform's own e2e suite (BCP-3734).
