...
* **New Resource:** `barndoor_mcp_server_publication` — publishes an MCP server, the one-way, admin-initiated step that makes it discoverable to end users. It is a separate resource (not a flag on `barndoor_mcp_server`) because publishing requires an ACTIVE policy, and policies reference the server's id — so the publication is ordered after them with `depends_on`. Re-applying is an idempotent no-op; removing the declaration does **not** unpublish (unpublishing does not exist) and never touches the server; import by server id. Requires platform support for the server publish endpoint (bdai-platform BCP-3659 / #6611) (BCP-3734).
